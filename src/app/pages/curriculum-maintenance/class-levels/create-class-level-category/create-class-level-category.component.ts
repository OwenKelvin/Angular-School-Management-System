...
export class CreateClassLevelCategoryComponent implements OnInit {
  ...
  validateName() {
    if (
      (this.classLevelCategoryForm.get('name').dirty || this.classLevelCategoryForm.get('name').touched) &&
  ...
  submit() {
    if (this.classLevelCategoryForm.valid) {
      this.classLevelCategory.submit(this.classLevelCategoryForm.value).subscribe(() => {
        if (this.newForm) {
          this.generateClassLevelCategoryForm();
